...
def _trace(
    ...
    mean = Point(intended.center.x + bias, intended.center.y)
    return tuple(
        PointerSample(
            t_ms=float(t_ms),
...
def evaluate(
    *,
    ...
    biases: Iterable[float] = DEFAULT_BIASES,
                ...
                for sigma in sigmas:
                    for bias in biases:
                        ...
                        for seed in range(seeds):
                            trace = _trace(intended, bias, sigma, cell_index * 100_000 + seed)
                            winner = bubble_solution(trace[-1].point, targets, config).primary
                            errors += int(winner is None or winner.id != intended.id)
    ...
    ambiguous = [cell for cell in cells if cell["baseline_error"] >= 0.20]
    ...
    gates = {
        "ambiguous_recall_at_least_0_80": trigger_recall is not None and trigger_recall >= 0.80,
